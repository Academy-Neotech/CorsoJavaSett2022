...
public interface PersonRepository extends JpaRepository<Person, Integer>, JpaSpecificationExecutor<Person>  {
	...
	///          SQL puro e val sul db
	@Query(value="select p.person_id as id,p.person_name as personName,mc.character_name as characterName from person p \n"
			+ "join movie_cast mc on p.person_id=mc.person_id \n"
			+ "join movie m on mc.movie_id=m.movie_id and\n"
			+ "m.movie_id=:movie_id",nativeQuery=true)
